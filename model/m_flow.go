...
func (a *Flow) GetStartNode(flowID string) (*schema.Node, error) {
	query := fmt.Sprintf("SELECT * FROM %s WHERE deleted=0 AND flow_id=? AND type_code=? ORDER BY order_num LIMIT 1", schema.NodeTableName)

	var item schema.Node
	err := a.DB.SelectOne(&item, query, flowID, "startEvent")
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (a *Flow) QueryTodo(typeCode, flowCode, userID string, count int) ([]*schema.FlowTodoResult, error) {
	...
	query := fmt.Sprintf(`
		...
		FROM %s ni
		 ...
		 JOIN %s n ON ni.node_id = n.record_id AND n.deleted = ni.deleted
	...
	query = fmt.Sprintf("%s ORDER BY ni.id DESC LIMIT %d", query, count)
...
func (a *Flow) QueryTodoPaginate(typeCode, flowCode, userID string, page int, pageSize int) (int, []*schema.FlowTodoResult, error) {
	...
	args = append(args, userID)
	if typeCode != "" {
		countQuery = fmt.Sprintf("%s AND fi.flow_id IN (SELECT record_id FROM %s WHERE deleted=0 AND flag=1 AND type_code=?)", countQuery, schema.FlowTableName)
		args = append(args, typeCode)
	} else if flowCode != "" {
		countQuery = fmt.Sprintf("%s AND fi.flow_id IN (SELECT record_id FROM %s WHERE deleted=0 AND flag=1 AND code=?)", countQuery, schema.FlowTableName)
		args = append(args, flowCode)
	}
	total, err := a.DB.SelectInt(countQuery, args...)
	if err != nil {
...
func (a *Flow) QueryDoneByPage(typeCode, flowCode, userID string, status, pageIndex, pageSize int) (int64, []*schema.FlowDoneResult, error) {
	...
	query := fmt.Sprintf("SELECT %s FROM %s %s ORDER BY ni.process_time DESC", fieldsSelect, table, where)

	count, err := a.DB.SelectInt(fmt.Sprintf("SELECT count(*) FROM %s %s", table, where), args...)
	if err != nil {
	...
	_, err = a.DB.Select(&items, query, args...)
	if err != nil {
		return 0, nil, errors.Wrapf(err, "查询用户的已办数据发生错误")
	}
...
func (a *Flow) QueryFlowIDsByType(typeCodes ...string) ([]string, error) {
	query := fmt.Sprintf("SELECT record_id FROM %s WHERE deleted=0 AND flag=1 AND status=1 AND type_code IN(?)", schema.FlowTableName)
	query, args, err := a.DB.In(query, typeCodes)
	if err != nil {
...
func (a *Flow) QueryFlowByIDs(flowIDs []string) ([]*schema.FlowQueryResult, error) {
	...
	// 批量查询
	flowResults, err := a.QueryFlowQueryResultByCodesAndVersions(codes, versions)
	if err != nil {
		return nil, errors.Wrapf(err, "根据流程ID查询流程数据发生错误")
	}
	...
	for i, item := range items {
		for _, fritem := range flowResults {
			if item.Code == fritem.Code && item.Version == fritem.Version {
				result[i] = fritem
				break
			}
		}
...
func (a *Flow) QueryNodeByTypeCodeAndFlowIDs(typeCode string, flowIDs ...string) ([]*schema.Node, error) {
	query := fmt.Sprintf("SELECT * FROM %s WHERE deleted=0 AND type_code=? AND flow_id IN(?)", schema.NodeTableName)
	query, args, err := a.DB.In(query, typeCode, flowIDs)
	if err != nil {
		return nil, errors.Wrap(err, "根据节点类型和流程ID列表查询节点数据发生错误")
	}
...
// 准备废弃，请使用（QueryNodeByTypeCodeAndFlowIDs）代替
...
func (a *Flow) QueryFlowQueryResultByCodesAndVersions(codes []string, versions []int64) ([]*schema.FlowQueryResult, error) {
	...
	query = fmt.Sprintf("%s WHERE deleted=0 AND flag=1 AND code IN(?) AND version IN(?)", query)

	query, args, err := a.DB.In(query, codes, versions)
	if err != nil {
		return nil, errors.Wrapf(err, "根据编号和版本列表查询流程结果数据发生错误")
	}
...
// 准备废弃，使用(QueryFlowQueryResultByCodesAndVersions)代替
